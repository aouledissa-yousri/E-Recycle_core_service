...
class CollectorService: 
    ...
    def createUserProfile(userType, request):
        collector = Collector()
        return collector.createUserProfile(userType, request)
    ...
    def signUp(request):
        ...
        collector = Collector()
        result = CollectorService.createUserProfile("generic", request)
        ...
        try: 
            ...
            if account.password == HashHelper.encryptPassword(credentials.password, account.salt) and (not account.isBlocked()):
                ...
                if not account.verified:
                    return CollectorService.sendConfirmationEmail(credentials.getData(), request)

                #check if two factor authentication is enabled
                elif account.twoFactorAuth: 
                    return CollectorService.sendTwoFactorAuthCode(credentials.getData(), request)
            

                #create session and get collector additional data
                return CollectorService.fetchCollectorData(account)
    ...
    def confirmAccount(request):
        ...
        try:
            ...
            if confirmationCode.expirationDate >= timezone.now():
                ...
                return CollectorService.fetchCollectorData(user)
    ...
    @staticmethod
    def fetchCollectorData(account: User):

        token = CollectorService.createSession(account)
    ...
    def requestPasswordReset(request):
        ...
        return CollectorService.sendPasswordResetCode(user, request)
